...
def set_environment(conf):
    ...
    print("#subprocess.call(grid_proxy_cmd)")
...
def write_to_remote(src, remote_filename):
    ...
    print('write to remote command: ' + str(cmd))
    try:
        print("#rc = subprocess.check_call(cmd)")
        print("#remote_obj_name = gen_remote_obj_name(pool=pool, token=token, rfname=remote_filename)")
...
def read_from_remote(remote_fn, dst):
    ...
    cmd = base_copy_cmd + [src, dst]
    print('read from remote command: ' + str(cmd))
    try:
...
if __name__ == '__main__':
    ...
    if not fallback_only:
        ...
        print("#returnsha1 = sha1_local_file(returnfname, tempfblksize)")
        # Compare SHA1s
